...
use hooks::{use_location, use_params_map, use_query_map};
...
#[component]
pub fn HomeActivity(ssr_site: Resource<Result<GetSiteResponse, AosAppError>>) -> impl IntoView {
  let i18n = use_i18n();
  ...
  let community_name = move || param.get().get("name").clone();

  let query = use_query_map();

  let ssr_list = move || serde_json::from_str::<ListingType>(&query.get().get("list").clone().unwrap_or("".into())).unwrap_or(ListingType::All);
  let ssr_sort = move || serde_json::from_str::<SortType>(&query.get().get("sort").clone().unwrap_or("".into())).unwrap_or(SortType::Active);
  let ssr_from = move || {
    serde_json::from_str::<(usize, Option<PaginationCursor>)>(&query.get().get("from").clone().unwrap_or("".into())).unwrap_or((0usize, None))
  };
  let ssr_prev =
    move || serde_json::from_str::<Vec<(usize, Option<PaginationCursor>)>>(&query.get().get("prev").clone().unwrap_or("".into())).unwrap_or(vec![]);
  let ssr_limit = move || query.get().get("limit").clone().unwrap_or("".into()).parse::<usize>().unwrap_or(10usize);
  ...
  let posts_resource = Resource::new(
    ...
    move |(_refresh, _logged_in, list_type, sort_type, from, limit, name)| async move {
      ...
      match result {
        ...
        Err(mut e) => {
          e.context = format!("Home Post Listing error - {}", e.context);
      ...
      //   loading.set(false);
      //   let e = AosAppError {
      //     context: "Home Post Listing error".into(),
  ...
  #[cfg(not(feature = "ssr"))]
  {
    ...
    use_resize_observer(_resize_element, move |entries, _| {
      ...
      let new_limit = if iw >= 2560f64 {
        query_params.insert("limit".to_string(), "40".to_string());
        Some("40".to_string())
      } else if iw >= 1920f64 {
      ...
      move |_entries, _| {
        ...
        if iw < 640f64 {
            ...
            && csr_resources.get().get(&(csr_next_page_cursor.get().0, ResourceStatus::Err)).is_none()
          {
            ...
            let _csr_resource = Resource::new(
              move || (),
              move |()| async move {
    ...
    <div class="block">
      ...
      <div class="hidden mr-3 sm:inline-block join">
        ...
        <A
          href={move || {
            let mut query_params = query.get();
            query_params.insert("list".to_string(), serde_json::to_string(&ListingType::Local).ok().unwrap());
            query_params.remove("from".into());
            query_params.remove("prev".into());
            format!("{}{}", use_location().pathname.get(), query_params.to_query_string())
          }}
          attr:class={move || format!("btn join-item{}", if ListingType::Local == ssr_list() { " btn-active" } else { "" })}
        >
          "Local"
        </A>
        <A
          ...
          attr:class={move || format!("btn join-item{}", if ListingType::All == ssr_list() { " btn-active" } else { "" })}
        >
          "All"
        </A>
      </div>
                  ...
                  <div class="hidden sm:block join">
                    {
                      ...
                      let mut query_params = query.get();
                      if st.len() > 0 {
                        query_params.insert("prev".to_string(), serde_json::to_string(&st).unwrap_or("[]".into()));
                      } else {
                        query_params.remove("prev".into());
                      }
                      if p.ne(&Some((0, None))) {
                        query_params.insert("from".to_string(), serde_json::to_string(&p).unwrap_or("[0,None]".into()));
                      } else {
                        query_params.remove("from".into());
                      }
                      view! {
                        <A
                          ...
                          attr:class={move || format!("btn join-item{}", if !ssr_prev().is_empty() { "" } else { " btn-disabled" })}
                        >
                          "Prev"
                        </A>
                      }.into_any()
                    }
                    {
                      ...
                      let mut query_params = query.get();
                      query_params.insert("prev".to_string(), serde_json::to_string(&st).unwrap_or("[]".into()));
                      query_params.insert("from".to_string(), serde_json::to_string(&next_page).unwrap_or("[0,None]".into()));
                      view! {
                        <A
                          on:click={move |_| {
                            loading.set(true);
        ...
        <For each={move || csr_resources.get()} key={|r| r.0.clone()} let:r>
          {
            ...
            view! {
              ...
              >
                <PostListings posts={r.1.clone().1.unwrap().posts.into()} ssr_site page_number={RwSignal::new(r.0.0)} />
              </Show>
            }
          }
        </For>
