use crate::{
  ...
  ui::components::{
    common::about::About,
    home::{page::Page, site_summary::SiteSummary, trending::Trending},
...
pub fn HomeActivity(ssr_site: Resource<Option<bool>, Result<GetSiteResponse, LemmyAppError>>) -> impl IntoView {
  ...
  let community_name = move || {
    ...
    // csr_next_page_cursor.set(None);
    param.get().get("name").cloned()
  };
  ...
  //     )
  //   },
  //   move |(_refresh, _logged_in, list_type, sort_type, from, limit, name, prevs)| async move {
  //     loading.set(true);
  //     // csr_next_page_cursor.set((0, None));
  //     // csr_resources.set(BTreeMap::new());
  ...
  //         // if lot_resources.get().get(&from.clone()).is_some() {
  //         //   logging::log!("cached!");
  //         // }

  //         // let ohye = o.clone();

  //         // lot_resources.update(move |lr| {
  //         //   // let mut i = 1usize;
  //         //   // for p in prevs {
  //         //   if lr.get(&(from.0.clone(), from.0.clone(), ResourceStatus::Loading)).is_none() {
  //         //     lr.insert(
  //         //       (from.0.clone(), from.0.clone(), ResourceStatus::Loading),
  //         //       (ohye.next_page.clone(), Some(ohye.clone())),
  //         //     );
  //         //   }
  //         //   //   i = i + 1;
  //         //   // }
  ...
  let lots_posts_resource = Resource::new(
    move || {
      (
        refresh.get(),
        logged_in.get(),
    ...
    move |(_refresh, _logged_in, list_type, sort_type, from, limit, name, prevs)| async move {
      ...
      if lot_resources.get().get(&from.clone()).is_some() {
        // logging::log!("cached!");
        Ok((from, None, lot_resources.get()))
      } else {
        // logging::log!("not");
        let form = GetPosts {
          type_: Some(list_type),
        ...
        // loading.set(false);
        match result {
          Ok(o) => Ok((from, Some(o), lot_resources.get())),
          Err(e) => {
            error.update(|es| es.push(Some((e.clone(), None))));
            Err((e, Some(refresh)))
          }
  ...
  #[cfg(not(feature = "ssr"))]
  {
    ...
    use_intersection_observer_with_options(
      _scroll_element,
      move |_entries, _| {
        if let Some(e) = lot_resources.get().last_entry() {
  ...
  // let lot_resources = expect_context::<
  //   RwSignal<BTreeMap<(usize, usize, ResourceStatus), Resource<Option<PaginationCursor>, Result<GetPostsResponse, LemmyAppError>>>>,
  // >();
  // // =
  // // RwSignal::new(BTreeMap::new());
  // // let lot_next_page_cursor: RwSignal<(usize, Option<PaginationCursor>)> = RwSignal::new((0, None));

  // let np = RwSignal::new((0, None::<PaginationCursor>));

  // let stop = watch(
  //   move || (query.get()),
  //   move |f, g, h| {
  //     let prevs =
  //       serde_json::from_str::<Vec<(usize, Option<PaginationCursor>)>>(&query.get().get("prev").cloned().unwrap_or("".into())).unwrap_or(vec![]);
  ...
  //       let mut i = 1usize;
  //       for p in prevs {
  //         if lr.get(&(i, p.0, ResourceStatus::Loading)).is_none() {
  //           lr.insert(
  //             (i, p.0, ResourceStatus::Loading),
  //             Resource::new(
  //               move || (p.1.clone()),
  //               move |(v)| async move {
  //                 let form = GetPosts {
  ...
  // let lengtho = Signal::derive(move || {
  //   if let Some(e) = lot_resources.get().last_entry() {
  //     if let Some(Ok(o)) = e.get().get() {
  //       Some(o.next_page)
  //     } else {
  ...
  //     None
  //   }
  //   // serde_json::from_str::<Vec<(usize, Option<PaginationCursor>)>>(&query.get().get("prev").cloned().unwrap_or("".into()))
  //   //   .unwrap_or(vec![])
  //   //   .len()
  // });
  ...
  view! {
    // <div> { move || lot_resources.get().len() } </div>
    ...
    <div>
      // <div> { move || format!("{:?}", np.get()) } </div>
      // <div> { move || format!("{:?}", lengtho.get()) } </div>
      //

      <Transition fallback={|| {}}>
        {move || {
          match lots_posts_resource.get() {
            Some(Ok(o)) => {
              let ohye = o.clone();
              // let woop = o.clone();
              // let goop = o.clone();
              ...
              view! {
                ...
                <div class={move || {
                  ...
                  <For each={move || lot_resources.get()} key={|r| r.0.clone() /* .1.0*/} let:r>
                    <PostListings posts={r.1.unwrap().posts.into()} ssr_site page_number={r.0.0.into()} on_community_change={move |s| {}} />
                  </For>
    ...
    //         match r.1.get() {
    //           Some(Ok(o)) => {
    //             // if r.0.0 == lot_resources.get().len() {
    //             // if r.0 == lengtho.get() {
    //             //             logging::log!("buu bu");
    //             // //       // logging::log!("luu lu {:?}", (r.0 * 10, o.next_page.clone()));
    //                 //   np.set((r.0.0 * 10, o.next_page));
    //                 // }
    //                 view! {
    //                   <span> "Ok" </span>
    //                 }
    ...
    // </div>
    // <div>
    //   <span> { move || format!("{:?}", np.get()) } </span>
    //   <For each={move || lots.get()} key={|r| r.0 /* .1.0*/} let:r>
    //     <Transition fallback={|| {}}>
    //       {move || {
    //         // let l = lots.get().len();
    //         match r.1.get() {
    ...
    //             // let ohye = posts.clone();

    //             // lot_resources.update(move |lr| {
    //             //   // let mut i = 1usize;
    //             //   // for p in prevs {
    //             //   if lr.get(&ohye.0).is_none() {
    //             //     lr.insert(ohye.0.clone(), Some(ohye.1.clone()));
    //             //   }
    //             //   //   i = i + 1;
    //             //   // }
    //             // });
    ...
    //                 format!("sm:block columns-1 2xl:columns-2 3xl:columns-3 4xl:columns-4 gap-0{}", if loading.get() { " opacity-25" } else { "" })
    //               }}>
    //                 <PostListings posts={posts.1.posts.into()} ssr_site page_number={posts.0.0.into()} on_community_change={move |s| {}} />
    //               </div>
    //               <div class="hidden sm:block join">
    //                 {
    //                   let mut st = ssr_prev();
